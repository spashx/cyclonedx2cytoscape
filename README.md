# CycloneDX to Cytoscape Converter (CdxViz)
...
![Example SBOM Visualization](example/CdxViz.gif)
...
```bash
CdxViz [options] <input-file> <output-file>
```
...
```bash
CdxViz path/to/sbom.json output.json
```

Include vulnerability information:
```bash
CdxViz --vulns path/to/sbom.json output.json
```

Include license information:
```bash
CdxViz --lic path/to/sbom.json output.json
```

Include both vulnerabilities and licenses:
```bash
CdxViz --vulns --lic path/to/sbom.json output.json
```

Include group names in node labels:
```bash
CdxViz --showGroupsInNodeLabels path/to/sbom.json output.json
```

Combine multiple options:
```bash
CdxViz --vulns --lic --showGroupsInNodeLabels path/to/sbom.json output.json
```

Generate VEX (Vulnerability Exploitability eXchange) report:
```bash
CdxViz --only-vex path/to/sbom.json output.json
```

Generate VDR (Vulnerability Disclosure Report) with affected components:
```bash
CdxViz --only-vdr path/to/sbom.json output.json
```
...
This section provides guidance on how to use Cytoscape to visualize and analyze the output files generated by the CdxViz tool.
...
- `test-dependency-tree-full.json.cyjs` - The corresponding Cytoscape.js output file generated using:
  ```bash
  CdxViz --vulns --lic example/test-dependency-tree-full.json example/test-dependency-tree-full.json.cyjs
  ```
...
This example demonstrates the full capabilities of the CdxViz tool, including:
